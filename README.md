...
2. 이 폴더 안의 파일을 **전부** (index.html, manifest.json, sw.js, icons 폴더 전체) 끌어다 놓기
...
3. 분석이 끝나면 `Android` 탭에서 패키지 생성 → 완료되면 `.apk`(또는 `.aab`) 파일을 다운로드
4. 그 파일을 폰으로 옮겨서 설치 (안드로이드에서 "출처를 알 수 없는 앱" 설치를 허용해야 할 수 있습니다)
